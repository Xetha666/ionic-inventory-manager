...
const AddProductModal: React.FC<AddProductModalProps> = ({ onProductAdded }) => {
  const {
    ...
    removeImage,
    handleSubmit,
  } = useAddProduct({ onSuccess: onProductAdded });
  ...
  return (
    <>
      ...
      <button
        ...
        onClick={openModal}
        className="fixed! bottom-24! right-container-padding! w-14 h-14 rounded-full! bg-primary text-on-primary flex items-center justify-center shadow-lg hover:bg-primary-tint active:scale-95 transition-all z-40 md:bottom-6! cursor-pointer"
        type="button"
      >
      ...
      {/* Modal */}
      <IonModal isOpen={isOpen} onDidDismiss={closeModal} className="add-product-modal">
        <div className="absolute inset-0 flex flex-col bg-surface-container-lowest overflow-hidden">
          ...
          <div className="flex items-center justify-between px-6 py-5 border-b border-outline-variant/10 shrink-0">
            ...
            <button onClick={closeModal} disabled={submitting} className="w-8 h-8 rounded-full! bg-surface-container hover:bg-surface-container-high flex items-center justify-center text-outline hover:text-on-surface transition-all cursor-pointer disabled:opacity-40" type="button" aria-label="Cerrar">
